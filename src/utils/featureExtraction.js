class FeatureExtractor {
  ...
  extractFeatures(text, mode = 'sentence') {
    const features = {};
  ...
  detectEmotionalContext(text) {
    return /\b(peur|crainte?|redoute?|anxiét|inquiét|effrai|joie|bonheur|colère|rage)\b/gi.test(text);
  }

  detectTemporalContext(text) {
    return /\b(avant|après|pendant|durant|temps|moment|tôt|tard)\b/gi.test(text);
  }

  detectLogicalContext(text) {
    return /\b(plus|moins|autant|parce\s+que|car|donc|si|condition)\b/gi.test(text);
  }
  ...
  detectSpeakerStance(text) {
    if (/\b(certainement|sûrement|évidemment|clairement)\b/gi.test(text)) {
      return 'assertive';
    }
    if (/\b(peut-être|probablement|sans\s+doute|il\s+semble)\b/gi.test(text)) {
      return 'tentative';
    }
    if (/\b(s'il\s+vous\s+plaît|veuillez|pourriez)\b/gi.test(text)) {
      return 'polite';
    }
    return 'neutral';
  }
  ...
  analyzeContextDepth(text) {
    const contextMarkers = [
      ...
      /\b(c'est-à-dire|autrement\s+dit|par\s+exemple)\b/gi
    ];
