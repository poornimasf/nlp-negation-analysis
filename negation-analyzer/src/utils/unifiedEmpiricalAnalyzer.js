...
class UnifiedEmpiricalAnalyzer {
  ...
  detectTrigger(text) {
    const patterns = {
      ...
      'sen_faut_que': /\b(?:(?:peu\s+)?s['']en\s+(?:est\s+)?(?:faut|fallut|fallu))\b/i,
  ...
  extractTriggerClause(text, trigger) {
    ...
    const triggerExtractionPatterns = {
      ...
      'sen_faut_que': /\b(?:il\s+)?s'en\s+(?:est\s+)?(?:faut|fallut|fallu|faudrait).*?(?:que?\s+([^.!?]*?))?(?:\s*\.|$)/i,
    ...
    if (triggerName === 'sen_faut_que' && /\bs'en\s+(?:est\s+)?(?:faut|fallut|fallu|faudrait)/i.test(text)) {
      try {
        const senFautMatch = text.match(/\b(?:il\s+)?s'en\s+(?:est\s+)?(?:faut|fallut|fallu|faudrait)[^.!?]*/i);
