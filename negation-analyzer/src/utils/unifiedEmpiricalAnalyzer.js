...
class UnifiedEmpiricalAnalyzer {
  ...
  detectTrigger(text) {
    const patterns = {
      ...
      'sen_faut_que': /\b(?:(?:peu\s+)?s['']en\s+(?:est\s+|était\s+|sera\s+|serait\s+|fut\s+|eût\s+)?(?:faut|fallut|fallu|fallait|faudra))\b/i,
  ...
  extractTriggerClause(text, trigger) {
    ...
    const triggerExtractionPatterns = {
      ...
      'sen_faut_que': /\b(?:il\s+)?s'en\s+(?:est\s+|était\s+|sera\s+|serait\s+|fut\s+|eût\s+)?(?:faut|fallut|fallu|fallait|faudra|faudrait).*?(?:que?\s+([^.!?]*?))?(?:\s*\.|$)/i,
    ...
    if (triggerName === 'sen_faut_que' && /\bs'en\s+(?:est\s+|était\s+|sera\s+|serait\s+|fut\s+|eût\s+)?(?:faut|fallut|fallu|fallait|faudra|faudrait)/i.test(text)) {
      try {
        const senFautMatch = text.match(/\b(?:il\s+)?s'en\s+(?:est\s+|était\s+|sera\s+|serait\s+|fut\s+|eût\s+)?(?:faut|fallut|fallu|fallait|faudra|faudrait)[^.!?]*/i);
