...
export const classifyExpletive = async (text) => {
  try {
    ...
    if (Array.isArray(result) && result.length > 0) {
      const analysis = result[0].generated_text;
      
      // Extract classification from analysis
      const classificationMatch = analysis.match(/Classification:\s*(EXPLETIVE|LOGICAL)/i);
      const classification = classificationMatch ? classificationMatch[1].toUpperCase() : 'UNCERTAIN';
      
      return {
        analysis,
        classification,
        confidence: classification !== 'UNCERTAIN' ? 0.85 : 0.5
      };
    }
